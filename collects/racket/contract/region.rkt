  ...
  (syntax-case stx ()
    ...
    [(_ id-info body0 body ...)
     (let ([expanded-body0 (local-expand #'body0
       ...
       ;; * Any auxillary definitions that should be introduced (contract and external id defs)
       ...
       (syntax-case expanded-body0 (begin define-values define-syntaxes)
         ...
         [(define-syntaxes (id ...) expr)
          (let ([ids (syntax->list #'(id ...))])
            (with-syntax ([(unused-ps def (newdef ...)) (rewrite-define #'define-syntaxes ids #'expr)])
                 (syntax/loc stx
                   (begin def (with-contract-helper unused-ps newdef ... body ...)))))]
         [(define-values (id ...) expr)
          (let ([ids (syntax->list #'(id ...))])
            (with-syntax ([(unused-ps def (newdef ...)) (rewrite-define #'define-values ids #'expr)])
              (syntax/loc stx
                (begin def (with-contract-helper unused-ps newdef ... body ...)))))]
         [else
          (quasisyntax/loc stx
